...
class Wowpays extends Model
{
    ...
    //代付回调(提现)
    public $notify_dai = 'https://api.alaoph.org/pay/wowpays/paydainotify';
    //代收回调(充值)
    public $notify_pay = 'https://api.alaoph.org/pay/wowpays/paynotify';
    //支付成功跳转地址    
    public $callback_url = 'https://www.alaoph.org/topupstatus/?orderid=';
    //代收秘钥
    public $key = "e4fa2826e2b4461dbed0fbaf0b2a4b1d";
    //代付秘钥
    public $daikey = "I3PNUBSYSDRIAJE5QGURFMUWVMAEWXQI";
    public function pay($order_id, $price, $userinfo, $channel_info)
    {
        ...
        Log::mylog("返回参数", $return_json, "wowpays");
    ...
    public function paynotify($params)
    {
        if ($params['tradeResult'] == 1) {
            ...
            if ($sign != $check) {
                Log::mylog('验签失败', $params, 'wowpayshd');
                return false;
            }
            $order_id = $params['mchOrderNo']; //商户订单号
            $order_num = $params['orderNo']; //平台订单号
            $amount = $params['amount']; //支付金额
            (new Paycommon())->paynotify($order_id, $order_num, $amount, 'wowpayshd');
        } else {
            //更新订单信息
            $upd = [
                'status' => 2,
            ...
            Log::mylog('支付回调失败！', $params, 'wowpayshd');
        }
    }
    ...
    public function withdraw($data, $channel)
    {
        ...
        Log::mylog('提现提交参数', $params, 'wowpaysdf');
        $return_json = $this->curls($params);
        Log::mylog($return_json, 'wowpaysdf', 'wowpaysdf');
        return $return_json;
    }
    ...
    public function paydainotify($params)
    {
        ...
        if ($sign != $check) {
            Log::mylog('验签失败', $params, 'wowpaysdfhd');
            return false;
        }
        ...
        if ($params['tradeResult'] != 1) {
            try {
                ...
                Log::mylog('代付失败,订单号:' . $params, 'wowpaysdfhd');
            } catch (Exception $e) {
                Log::mylog('代付失败,订单号:' . $params['merTransferId'], $e, 'wowpaysdfhd');
            }
        } else {
            try {
                $r = $usercash->where('order_id', $params['merTransferId'])->find()->toArray();
                ...
                Log::mylog('提现成功', $params, 'wowpaysdfhd');
            } catch (Exception $e) {
                Log::mylog('代付失败,订单号:' . $params['merTransferId'], $e, 'wowpaysdfhd');
            }
        }
    ...
    public function generateSign(array $params, $key)
    {
        ...
        Log::mylog('验签串', $params_str, 'wowpays');
