...
class Nicepaytwo extends Model
{
    //代付提单url(提现)
    ...
    //代付回调(提现)
    public $notify_dai = 'https://api.rothpro.id/pay/nicepaytwo/paydainotify';
    //代收回调(充值)
    public $notify_pay = 'https://api.rothpro.id/pay/nicepaytwo/paynotify';
    //支付成功跳转地址    
    public $callback_url = 'https://www.rothpro.id/topupsuccess.html';
    //代收秘钥
    public $key = "01eaac6aca3ddfc7aa9cd561dff2c6b4";
    //代付秘钥
    public function pay($order_id, $price, $userinfo, $channel_info)
    {
        if($channel_info['busi_code'] == 'Online'){
            $p_method = '';
        }else{
            $p_method = 'QRIS';
        }
        ...
        $param['sign'] = $sign;
        Log::mylog("提交参数", $param, "Nicepay2");
        $return_json = $this->fetch_page_json($this->pay_url, $param);
        Log::mylog("返回参数", $return_json, "Nicepay2");
    ...
    public function paynotify($params)
    {
        if ($params['status'] == 1) {
            ...
            if ($sign != $check) {
                Log::mylog('验签失败', $params, 'Nicepay2hd');
                return false;
            }
            $order_id = $params['order']; //商户订单号
            $order_num = ""; //平台订单号
            $amount = $params['amount']; //支付金额
            (new Paycommon())->paynotify($order_id, $order_num, $amount, 'Nicepay2hd');
        } else {
            //更新订单信息
            $upd = [
                'status' => 2,
            ...
            Log::mylog('支付回调失败！', $params, 'Nicepay2hd');
        }
    }
...
//            'bankCode' => 'GCASH',
        ...
        $sign = $this->sendSign($param, $this->key);
        $param['sign'] = $sign;
        Log::mylog('提现提交参数', $param, 'Nicepay2df');
        $return_json = $this->fetch_page_json($this->dai_url, $param);
        Log::mylog($return_json, 'Nicepay2df', 'Nicepay2df');
        return $return_json;
    }
    ...
    public function paydainotify($params)
    {
        ...
        $check = $this->sendSign($params, $this->key);
        if ($sign != $check) {
            Log::mylog('验签失败', $params, 'Nicepay2dfhd');
            return false;
        }
        ...
        if ($params['status'] != 1) {
            try {
                ...
                Log::mylog('代付失败,订单号:' . $params['order'], 'Nicepay2dfhd');
            } catch (Exception $e) {
                Log::mylog('代付失败,订单号:' . $params['order'], $e, 'Nicepay2dfhd');
            }
        } else {
            try {
                $r = $usercash->where('order_id', $params['order'])->find()->toArray();
                ...
                Log::mylog('提现成功', $params, 'Nicepay2dfhd');
            } catch (Exception $e) {
                Log::mylog('代付失败,订单号:' . $params['order'], $e, 'Nicepay2dfhd');
            }
        }
