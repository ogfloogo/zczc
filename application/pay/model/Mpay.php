...
class Mpay extends Model
{
    ...
    //代付回调(提现)
    public $notify_dai = 'https://api.alaoph.org/pay/mpay/paydainotify';
    //代收回调(充值)
    public $notify_pay = 'https://api.alaoph.org/pay/mpay/paynotify';
    //代收秘钥
    public $key = "ef816ffc4139448fb07d399b4e97b858";
    //代付秘钥
    public function pay($order_id, $price, $userinfo, $channel_info)
    {
        ...
        $param['sign'] = $sign;
        Log::mylog("提交参数", $param, "Mpay");
        $header[] = "Content-Type: application/json;charset=utf-8";
        $return_json = $this->httpPost($this->pay_url, $param);
        Log::mylog("返回参数", $return_json, "Mpay");
    ...
    public function paynotify($params)
    {
        if ($params['payResult'] == 1) {
            ...
            Log::mylog('支付回调失败！', $params, 'Mpayhdhd');
        }
    }
    ...
    public function withdraw($data, $channel)
    {
        ...
        $sign = $this->sendSign($param, $this->key);
        $param['sign'] = $sign;
        Log::mylog('提现提交参数', $param, 'Mpaydf');
        $header[] = "Content-Type: application/json;charset=utf-8";
        $return_json = $this->httpPost($this->dai_url, $param);
        Log::mylog($return_json, 'Mpaydf', 'Mpaydf');
        return $return_json;
    }
    ...
    public function paydainotify($params)
    {
        ...
        if ($params['payResult'] != 1) {
            try {
                ...
                Log::mylog('代付失败,订单号:' . $params['orderNo'], 'Mpaydfhd');
            } catch (Exception $e) {
                Log::mylog('代付失败,订单号:' . $params['orderNo'], $e, 'Mpaydfhd');
            }
        } else {
            try {
                $r = $usercash->where('order_id', $params['orderNo'])->find()->toArray();
                ...
                Log::mylog('提现成功', $params, 'Mpaydfhd');
            } catch (Exception $e) {
                Log::mylog('代付失败,订单号:' . $params['order_no'], $e, 'Mpaydfhd');
            }
        }
    }
