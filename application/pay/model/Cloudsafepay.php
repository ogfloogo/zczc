...
class Cloudsafepay extends Model
{
    ...
    //代付回调(提现)
    public $notify_dai = 'https://api.alaoph.org/pay/cloudsafepay/paydainotify';
    //代收回调(充值)
    public $notify_pay = 'https://api.alaoph.org/pay/cloudsafepay/paynotify';
    //支付成功跳转地址    
    ...
    public function paydainotify($params)
    {
        ...
        if ($params['tradeState'] != "PAIED") {
            try {
                ...
                Log::mylog('代付失败,订单号:' . $params['orderNo'], 'Cloudsafepaydfhd');
            } catch (Exception $e) {
                Log::mylog('代付失败,订单号:' . $params['orderNo'], $e, 'Cloudsafepaydfhd');
            }
        } else {
            try {
                $r = $usercash->where('order_id', $params['orderNo'])->find()->toArray();
