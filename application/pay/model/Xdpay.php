...
class Xdpay extends Model
{
    //代付提单url(提现)
    public $dai_url = 'https://apis.xdpay168.com/client/pay/create';
    //代收提交url(充值)
    ...
    public function paynotify($params)
    {
        if ($params['status'] == 1) {
            ...
            if ($sign != $check) {
                Log::mylog('验签失败', $params, 'Xdpayhd');
                return false;
            }
            $order_id = $params['orderId']; //商户订单号
            $order_num = $params['platOrderId']; //平台订单号
            $amount = $params['amount']; //支付金额
            (new Paycommon())->paynotify($order_id, $order_num, $amount, 'Xdpayhdhd');
        } else {
            //更新订单信息
            $upd = [
                'status' => 2,
            ...
            Log::mylog('支付回调失败！', $params, 'Xdpayhdhd');
        }
    }
    ...
    public function paydainotify($params)
    {
        ...
        if ($params['status'] != 1) {
            try {
                ...
                Log::mylog('代付失败,订单号:' . $params['orderId'], 'Xdpaydfhd');
            } catch (Exception $e) {
                Log::mylog('代付失败,订单号:' . $params['orderId'], $e, 'Xdpaydfhd');
            }
        } else {
            try {
                $r = $usercash->where('order_id', $params['orderId'])->find()->toArray();
                ...
                Log::mylog('提现成功', $params, 'Xdpaydfhd');
            } catch (Exception $e) {
                Log::mylog('代付失败,订单号:' . $params['orderId'], $e, 'Xdpaydfhd');
            }
        }
