...
class Gtrpays extends Model
{
    ...
    //代付回调(提现)
    public $notify_dai = 'https://api.taya777.cloud/pay/gtrpays/paydainotify';
    //代收回调(充值)
    public $notify_pay = 'https://api.taya777.cloud/pay/gtrpays/paynotify';
    //代收秘钥
    public $key = "01b8f80fc5e64890b2a0be4c430a4d81";
    //代付秘钥
    ...
    public function paynotify($params)
    {
        if ($params['payStatus'] == 1) {
            ...
            if ($sign != $check) {
                Log::mylog('验签失败', $params, 'Gtrpayshd');
                return false;
            }
            $order_id = $params['orderNo']; //商户订单号
            $order_num = $params['tradeNo']; //平台订单号
            $amount = $params['realAmount']; //支付金额
            (new Paycommon())->paynotify($order_id, $order_num, $amount, 'Gtrpayshd');
        } else {
            //更新订单信息
            $upd = [
                'status' => 2,
    ...
    public function paydainotify($params)
    {
        ...
        if ($params['payStatus'] != 1) {
            try {
                ...
                Log::mylog('代付失败,订单号:' . $params['orderNo'], 'Gtrpaysdfhd');
            } catch (Exception $e) {
                Log::mylog('代付失败,订单号:' . $params['orderNo'], $e, 'Gtrpaysdfhd');
            }
        } else {
            try {
                $r = $usercash->where('order_id', $params['orderNo'])->find()->toArray();
                ...
                Log::mylog('提现成功', $params, 'Gtrpaysdfhd');
            } catch (Exception $e) {
                Log::mylog('代付失败,订单号:' . $params['orderNo'], $e, 'Gtrpaysdfhd');
            }
        }
