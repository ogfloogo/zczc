 ...
 * 转换时间
 */
function format_date_str($timeStamp)
{
    $time = $timeStamp;
...
function format_image($url)
{
    ...
    foreach ($url as $value) {
        if (strpos($value, "http") !== false) {
            ...
            $return .= Config::get('image_url') . $value . ',';
        }
    }
    return rtrim($return, ',');
}
 ...
 * @param $array 排序二维数组
 * @param $array 排序字段
 * @param $sort SORT_DESC 降序  SORT_ASC 升序
 */
function setArraysort($array, $param, $sort)
{
    $res = array_column($array, $param);
    if($sort == "asc"){
        array_multisort($res, SORT_ASC, $array);
    }elseif($sort == "desc"){
...
function robotimage()
{
    ...
    $dirname = ROOT_PATH . 'public/uploads/ff'; //要遍历的目录名字
    $dh = opendir($dirname);
    $file = readdir($dh);
    // dump($file);exit;
    $i = 1;
    ...
    while ($file = readdir($dh)) {
        ...
        $a = rand(10, 99);
        $b = rand(100, 999);
        $img_url = date('YmdHis') . rand(10000000, 99999999);
        if (3 < strlen($name[0]) && 50 > strlen($name[0])) {
            // $insert['data'][] =[
            //     //$begin.$a.'****'.$b,
            //     $name[0],
            //     $begin.$a.$i,
            ...
            //     time(),
            // ];
            $userrobot->insert(['name' => $name[0], 'avatar' => '/uploads/robotpic/' . $i . '.jpg']);
            copy(ROOT_PATH . '/public/uploads/ff/' . $file, ROOT_PATH . '/public/uploads/robotpic/' . $i . '.jpg');
        }
    }
}
...
{
    //随机电话号段
    $my_array = array(811,812,813,821,822,823,851,852,853,814,815,816,855,858,856,857,895,896,897,898,899,817,818,819,877,831,832,833,838,881,882,883,884,885,886,887,888,889);
