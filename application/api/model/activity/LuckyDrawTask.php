...
class LuckyDrawTask extends BaseModel
{
    protected function initialize()
    {
        parent::initialize();
        $prefix_config = Config::get('activity.redis_key_prefix')['lucky_draw_task'];
    ...
    {
        $set_key = $this->set_prefix;
        $level_list = $this->redisInstance->zRevRange($set_key, 0, -1, true);
        $list = [];
    ...
    public function formatInfo($id, $user_info)
    {
        $detail_info = $this->info($id, $user_info);
        $info = [];
        if ($detail_info) {
            $info['name'] = $detail_info['name'];
