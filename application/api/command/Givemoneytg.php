...
class Givemoneytg extends Command
{
    ...
    {
        // 指令配置
        $this->setName('Givemoneytg')
            ->setDescription('理财到期-推广项目');
    }

    protected function execute(Input $input, Output $output)
    {
        // 指令输出
        $output->writeln('Givemoneytg');
        $ws = new \Swoole\WebSocket\Server('0.0.0.0', 9518);
        //守护进程模式
        $ws->set([
            'daemonize' => true,
            'worker_num' => 1,
        ...
        $ws->on('WorkerStart', function ($ws, $worker_id) {
            ...
            \Swoole\Timer::tick(1500, function () use ($redis, $user, $usermoneylog, $worker_id) {
                ...
                $list = $redis->handler()->ZRANGEBYSCORE('zclc:financelisttg', '-inf', time(), ['withscores' => true]);
                foreach ($list as $key => $value) {
                    $lock = $redis->handler()->setnx("zclc:financelocktg" . $key, $key);
                    if (!$lock) {
                        continue;
                    }
                    $redis->handler()->expireAt("zclc:financelocktg" . $key, time() + 10);
                    //理财订单是否存在
                    $order_id = $key;
                    $order_info = (new Financeorder())->where(['id'=>$order_id])->find();
                    if(!$order_info){
                        Log::mylog('理财订单不存在', $list, 'Givemoneytg');
                        $redis->handler()->del("zclc:financelocktg" . $key);
                        $redis->handler()->zRem('zclc:financelisttg', $key);
                        continue;
                    }
                    ...
                    if($order_info['status'] == 2){
                        Log::mylog('订单已发放', $list, 'Givemoneytg');
                        $redis->handler()->del("zclc:financelocktg" . $key);
                        $redis->handler()->zRem('zclc:financelisttg', $key);
                        continue;
                    }
                    try {
                        ...
                        if($surplus_num-1 == 0){
                            ...
                            $redis->handler()->del("zclc:financelocktg" . $key);
                            //加入下一期的结算时间
                            $redis->handler()->zAdd("zclc:financelisttg", $collection_time, $order_id);
                            $order_info->save();
                            ...
                            $redis->handler()->rpush("commissionlisttg", $commission);
                            continue;
                        }
