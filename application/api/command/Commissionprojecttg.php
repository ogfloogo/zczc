...
class Commissionprojecttg extends Command
{
    ...
    {
        // 指令配置
        $this->setName('Commissionprojecttg')
            ->setDescription('推广项目-发放');
    }

    protected function execute(Input $input, Output $output)
    {
//        // 指令输出
        $output->writeln('Commissionprojecttg');
        ...
        $ws->on('WorkerStart', function ($ws, $worker_id) {
            ...
            \Swoole\Timer::tick(3000, function () use ($redis, $commission, $usermoneylog, $usertotal, $order, $team_level) {
                ...
                for ($i = 0; $i < 100; $i++) {
                    Db::startTrans();
                    try {
                        $value = $redis->handler()->lpop('commissionlisttg');
                        if ($value) {
                            $list = explode("-", $value);
                            $from_id = $list[0]; //来源ID
