...
class Givemoneyty extends Command
{
    ...
    {
        // 指令配置
        $this->setName('Givemoneyty')
            ->setDescription('理财到期-体验项目');
    }

    protected function execute(Input $input, Output $output)
    {
        // 指令输出
        $output->writeln('Givemoneyty');
        ...
        $ws->on('WorkerStart', function ($ws, $worker_id) {
            ...
            \Swoole\Timer::tick(1500, function () use ($redis, $user, $usermoneylog, $worker_id) {
                ...
                $list = $redis->handler()->ZRANGEBYSCORE('zclc:financelistty', '-inf', time(), ['withscores' => true]);
                foreach ($list as $key => $value) {
                    $lock = $redis->handler()->setnx("zclc:financelockty" . $key, $key);
                    if (!$lock) {
                        continue;
                    }
                    $redis->handler()->expireAt("zclc:financelockty" . $key, time() + 10);
                    //理财订单是否存在
                    $order_id = $key;
                    $order_info = (new Financeorder())->where(['id'=>$order_id])->find();
                    if(!$order_info){
                        Log::mylog('理财订单不存在', $list, 'Givemoneyty');
                        $redis->handler()->del("zclc:financelockty" . $key);
                        $redis->handler()->zRem('zclc:financelistty', $key);
                        continue;
                    }
                    ...
                    if($order_info['status'] == 2){
                        Log::mylog('订单已发放', $list, 'Givemoneyty');
                        $redis->handler()->del("zclc:financelockty" . $key);
                        $redis->handler()->zRem('zclc:financelistty', $key);
                        continue;
                    }
                    try {
                        ...
                        if($surplus_num-1 == 0){
                            ...
                            $redis->handler()->del("zclc:financelockty" . $key);
                            //加入下一期的结算时间
                            $redis->handler()->zAdd("zclc:financelistty", $collection_time, $order_id);
                            $order_info->save();
                            ...
                            $redis->handler()->rpush("commissionlistty", $commission);
                            continue;
                        }
